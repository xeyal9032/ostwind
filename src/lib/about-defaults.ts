export const EMPTY_LOCALES = { tr: '', en: '', az: '', ru: '', uk: '', ge: '' };

export const DEFAULT_STORY_IMAGE =
  'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&q=80&w=800';

export const DEFAULT_COMPANY_SECTION = {
  companyTitle: {
...
export function mergeLocaleJson(field: unknown) {
  if (!field) return { ...EMPTY_LOCALES };
  return { ...EMPTY_LOCALES, ...(typeof field === 'string' ? JSON.parse(field) : (field as Record<string, string>)) };
}

const LOCALE_KEYS = ['tr', 'en', 'az', 'ru', 'uk', 'ge'] as const;

export function fillEmptyLocales(
  value: unknown,
  defaults: Record<string, string>
): Record<string, string> {
  const merged = sanitizeLocaleJson(value);
...
export function sanitizeLocaleJson(value: unknown): Record<string, string> {
  const result: Record<string, string> = { ...EMPTY_LOCALES };
...
export function getLocaleText(field: Record<string, string> | string | null | undefined, locale: string) {
  if (!field) return '';
  if (typeof field === 'string') return field;
  if (field[locale]?.trim()) return field[locale];
  return field.tr || field.en || field.az || Object.values(field).find((v) => v?.trim()) || '';
}
