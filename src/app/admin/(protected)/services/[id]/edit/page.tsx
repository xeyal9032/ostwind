      ...
      try {
        ...
        if (res.ok) {
          ...
          const mergeJson = (field: unknown, localeOnly = false) => {
            ...
            const raw =
              typeof field === 'string'
                ? (JSON.parse(field) as Record<string, string>)
                : (field as Record<string, string>);
            if (!localeOnly) {
              return { ...base, ...raw };
            }
            LOCALES.forEach((lang) => {
              if (raw[lang.code]) base[lang.code as keyof typeof base] = raw[lang.code];
            });
            return base;
          };
          ...
          setFormData({
            slug: data.slug || '',
            icon: normalizeServiceIconForAdmin(data.icon, data.slug),
            title: mergeJson(data.title),
            description: mergeJson(data.description, true),
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 shadow-sm rounded-xl border border-gray-200 dark:border-zinc-800 p-6">
        ...
        <div className="mb-8 pb-8 border-b border-gray-200 dark:border-zinc-800 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <ServiceIconSelect
            value={formData.icon}
            onChange={(icon: ServiceIconKey) => setFormData({ ...formData, icon })}
          />
        </div>
